...
class ODCRN(nn.Module):
    def __init__(self, num_nodes:int, K:int, input_dim:int, hidden_dim:int, out_horizon:int, num_layers:int, device, data, DGCbool:bool=True, use_bias=True, activation=None):
        ...
        A_neighbor = np.expand_dims(np.load(A_path), axis=0)
        A_neighbor = torch.from_numpy(A_neighbor).repeat(K, 1, 1)
        self.A_neighbor = A_neighbor.float().to(device)
    
    def forward(self, X_seq:torch.Tensor, G:torch.Tensor, x_timeF, y_timeF):
