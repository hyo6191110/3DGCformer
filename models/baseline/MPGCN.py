...
class MPGCN(nn.Module):
    def __init__(self, M:int, K:int, input_dim:int, lstm_hidden_dim:int, lstm_num_layers:int, gcn_hidden_dim:int, gcn_num_layers:int,
                 num_nodes:int, out_horizon:int, user_bias:bool, device, data, activation=None):
    ...
    def forward(self, x_seq: torch.Tensor, G_list:list, x_timeF, y_timeF):
        '''
        ...
        # [B, T, H, W, 1] -> [B, T, H, W]
        ensemble_out = ensemble_out.squeeze(4)
        return ensemble_out
